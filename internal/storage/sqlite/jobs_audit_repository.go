...
func (s *Store) CompleteJob(ctx context.Context, id int64, owner string, at time.Time) error {
	query := `UPDATE worker_jobs SET status = 'succeeded', lease_owner = '', lease_until = NULL, updated_at = ? WHERE id = ? AND status = 'leased' AND lease_owner = ?`
	args := []any{formatTime(at), id, owner}
	if owner == "" {
		query = `UPDATE worker_jobs SET status = 'succeeded', lease_owner = '', lease_until = NULL, updated_at = ? WHERE id = ? AND status = 'leased'`
		args = []any{formatTime(at), id}
	}
...
func (s *Store) MergeNotificationReceipt(ctx context.Context, providerKey, status string, at time.Time) error {
	result, err := s.exec.ExecContext(ctx, `UPDATE notification_deliveries SET
		status = CASE
			...
			WHEN status = 'sent' AND ? = 'failed' THEN status
			ELSE ? END,
		last_receipt_at = CASE WHEN last_receipt_at IS NULL OR last_receipt_at < ? THEN ? ELSE last_receipt_at END,
		updated_at = ? WHERE provider_key = ?`, status, status, status, formatTime(at), formatTime(at), formatTime(at), providerKey)
	if err != nil {
